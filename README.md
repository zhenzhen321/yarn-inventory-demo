...
- 二维码出库：一批一码、一批一张标签；扫码枪扫入完整批次码即自动加入本单（无需回车或点击），批次在其他仓库且本单无明细时自动切换仓库；也支持按内部批次号手工选择
...
- 工程质量：193 个自动化测试（34 个测试文件）、自包含端到端验收脚本、GitHub Actions 持续集成、数据库备份/恢复脚本
...
4. 打开“卖出出库”，先选择仓库，再把扫码枪焦点放在“扫码枪快速出库”输入框。扫码枪应使用 USB HID 键盘模式；扫入完整批次码即自动加入本单并回焦输入框，可连续扫码，无需回车或点击。
5. 没有扫码枪时，可把库存页显示的 `LOT-*` 内部批次号输入同一个扫码框（输入完整码后同样自动加入），或点击“加入”按钮，演示相同的自动选货、整批带出和重复扫码拦截流程；扫描其他仓库的批次时，本单无明细会自动切换仓库并提示。
