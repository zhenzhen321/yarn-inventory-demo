...
- 加工追溯：原料送厂 → 加工消耗 → 成品新批次 → 回仓或染厂直销，保留投入、产出、损耗、成本和父子关系
...
- 工程质量：155 个自动化测试（26 个测试文件）、自包含端到端验收脚本、GitHub Actions 持续集成、数据库备份/恢复脚本
...
端到端脚本自包含、可重复运行：登录 → 建测试数据 → 买入并生成内部批次 → 卖出批次分配 → 调拨/送加工 → 加工产出新批次 → 加工收回/盘库 → 结算与超结拦截 → 撤回与反向流水 → 各类 Excel 导出，验证后可用 `node scripts/cleanup-e2e.js` 清理测试数据。
