...
- 库存引擎：买入、卖出、多仓调拨、盘库和撤回全部事务化；版本号防止并发重复扣减
- 批次溯源：每条采购明细形成独立内部批次，同品种、同供应商批号再次采购也不合并成本层
- 二维码出库：一批一码、一批一张标签；支持二维码扫码枪回车录入，也支持按内部批次号快速选择
- 加工追溯：原料送厂 → 加工消耗 → 成品新批次 → 回仓或染厂直销，保留投入、产出、损耗、成本和父子关系；支持多投入多产出与成本分配
- 幂等写入：采购、销售、调拨、盘库、加工完工和结算等写操作支持 Idempotency-Key，网络重试不会重复记账
- 家庭点选界面：常用仓库/往来单位/规格快捷点选、大字模式、业务日期快捷项、加工分步核对和未保存提醒
...
- 工程质量：183 个自动化测试（32 个测试文件）、自包含端到端验收脚本、GitHub Actions 持续集成、数据库备份/恢复脚本
...
| 数据层 | Prisma ORM + SQLite、Prisma Migrations、不可变库存流水 |
| 安全 | bcryptjs、jose（JWT）、登录限速、CSRF Origin 校验 |
| 测试 | Vitest（单元测试）、自研 e2e 脚本（端到端验收） |
| 其他 | qrcode（本地二维码生成）、ExcelJS（Excel 导出） |
...
| 阶段 | 文档 | 产出 |
| --- | --- | --- |
| 1. 需求分析 | [docs/01-需求分析/需求与目标.md](docs/01-需求分析/需求与目标.md) | 从业务场景拆解功能点 F1-F16，明确范围、角色与决策 |
| 2. 设计文档 | [批次溯源与扫码出库设计](docs/02-设计文档/批次溯源与扫码出库设计.md) | 货源批次、分地点余额、库存流水、扫码标签和加工血缘 |
...
种子数据包含 3 个仓库、3 家往来单位、3 个纱线产品（6 个规格）、4 笔买入、5 个内部批次、12 条库存流水、2 笔卖出、2 笔调拨、1 条完整加工链、1 次盘库和资金结算。两笔纯棉采购故意使用相同供应商批号、不同日期和价格，用来展示内部批次与成本层不会合并。

> `admin / demo123456` 与 `clerk / demo123456` 仅用于本地演示。公开部署前请通过环境变量设置新密码，并替换 `SESSION_SECRET`。

## 批次溯源与扫码出库演示

1. 打开“买入入库 → 买入记录”，展开单据并点击“打印标签”，查看内部批次号和二维码预览。
2. 打开“库存”，展开来源单据。相同供应商批号 `C20260801` 会显示两个不同的 `LOT-*` 内部批次。
3. 点击内部批次号进入追溯页，查看采购来源、当前位置、库存流水、销售客户，以及加工前后父子批次。
4. 打开“卖出出库”，先选择仓库，再把扫码枪焦点放在“扫码枪快速出库”输入框。扫码枪应使用 USB HID 键盘模式，并在扫码后发送 Enter。
5. 没有扫码枪时，可把库存页显示的 `LOT-*` 内部批次号输入同一个扫码框并回车，演示相同的自动选货、整批带出和重复扫码拦截流程。
6. 部分销售时修改重量和件数后再保存；提交时服务端会重新校验地点、版本和余额，二维码只负责定位，不绕过库存规则。

二维码只包含随机扫描令牌，不包含采购成本。标签默认一批一张；真实打印机、标签纸和扫码枪仍需在具体设备上校准。
...
npm test              # 183 个自动化测试（32 个文件）
npm run typecheck     # 类型检查
npm run build         # 生产构建
npm run test:e2e      # 隔离 HTTP 端到端（自动建 e2e.db、3100 端口并清理）
```

端到端脚本通过 `npm run test:e2e` 在隔离的 `prisma/e2e.db` 和 `127.0.0.1:3100` 中运行，自动迁移、seed、构建和清理；运行前后会比较 `prisma/dev.db` 哈希，拒绝触碰家庭业务库。流程覆盖登录、幂等采购重试、批次销售/调拨、加工费、撤回、导出、两批投入两批产出加工及相同幂等键重放。脚本结束会清理 E2E 前缀数据；禁止直接运行 `node scripts/e2e.mjs`。
...
- 页面顶部/手机底部提供首页、库存、买入、卖出和更多入口；可打开“大字”模式，常用仓库、往来单位和品种提供快捷按钮。
- 买入、卖出、调拨、加工完工和结算支持今天/昨天等业务日期快捷项；加工完工按投入、产出、费用三步核对。
- 加工完工可添加多个投入批次和多个成品批次，服务端按产出权重分配货物成本、运费、加工费和其他费用，最后一项承接分币尾差。
- 保存按钮在请求期间锁定；网络异常时保留原表单并重试，服务端按 Idempotency-Key 返回第一次结果。
